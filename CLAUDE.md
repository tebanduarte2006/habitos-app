...
| 2026-04-28 | `b948d34` | **Tab Ejercicios > Detalle:** botón `✏️ Editar músculos` en la vista detalle del ejercicio que abre modal con el mismo search-picker de "Crear ejercicio" preseleccionando los músculos actuales + confirmación antes de guardar (`gymOpenEditMusclesModal`). Afecta retroactivamente el historial. **Refactor:** picker extraído como `gymBuildMusclePicker(opts)` reutilizable (Crear + Editar). **Búsqueda accent/case-insensitive:** nueva helper `gymNormalizeKey()` (NFD + strip diacritics + lower) aplicada al buscador del directorio Ejercicios, autocomplete de rutinas (Iniciar sesión + Crear ejercicio), buscador de ejercicios en sesión activa y dedup del muscle picker → buscar `triceps` ahora encuentra `Tríceps`. **Auditoría GYM_MUSCLE_GROUPS:** sin duplicados exactos; los 5 generales (Pecho/Espalda/Hombros/Core/Piernas) son hipernimos válidos, se mantienen. **Verificación:** simulación de roundtrip export/import en Node con datos sintéticos → 10/10 tests OK (preserva flow_*/preferencias, valida JSON malformado, fusión por ID confirmada). `sw.js → habitos-20260428-1`. |
| 2026-04-27 | `1eb3a5f` | **Tab Progresión — rework total:** vista lista de ejercicios (más reciente primero, sin historial al final en gris) → detalle por ejercicio con (a) PR card con peso absoluto + reps + fecha + 1RM estimado Epley, (b) gráfica SVG inline (área + línea + puntos, últimas 10 sesiones, peso máx), (c) lista de últimas 10 sesiones como `<details>` colapsables con sets marcados con badge 🏆 PR cuando fueron récord en ese momento (running max). Eliminada la sección "Sesiones completadas" (remanente Benny). Export/Import preservados al final del tab; verificados con simulación de roundtrip JSON + math PR/1RM Epley + PR-tracking + scope tx (no toca flow_*) → 22/22 tests pasaron. Nuevas funciones: `gymBuildProgRow`, `gymBuildSvgChart`, `gymBuildSessionDetails`. **Skill nueva (global):** `~/.claude/skills/habitos-app-workflow/SKILL.md` que se activa en cualquier conversación cuando el usuario pide cambios sobre la PWA Hábitos; impone entrevista exhaustiva proporcional al tamaño + recomendaciones + checklist + bumpeo de sw.js + changelog + commit/push. Reemplaza la skill local `habitos-changelog` (eliminada). `sw.js → habitos-20260427-2`. |
| 2026-04-28 | `f4de005` | **Rediseño visual completo · Apple HIG dark (Claude Design V3 → vanilla port).** Output de Claude Design (3 iteraciones, React/JSX) porteado al stack vanilla. Cambios principales: **(a) `styles.css` reescrito** con design tokens (`--accent`, `--bg2..4`, `--r-md/lg/xl`, `--pill`) y nuevas clases `g-*` / `h-*` / `m-*`; preserva clases `.mental-*` legacy con look refrescado. **(b) Home rediseñado** (`renderers/home.js` nuevo): grid 2-col de cards de módulos + card "Esta semana" con métricas reales calculadas en runtime de IndexedDB (count + suma `duracion_ms` + última actividad relativa). Cero metas/rachas inventadas. **(c) `renderers/icons.js` nuevo** con builders SVG vanilla. **(d) Gym rediseño total** preservando 100% lógica DB: tabs Apple-style, session card con label DURACIÓN, rest timer bar con progreso + Saltar, exercise cards collapsibles con count Done/total, set rows compactos con chip Pendiente/Hecho/Saltado coloreado, modales bottom-sheet con handle + close button. **Hero "Récord de la semana"** agregado al inicio del Tab Progresión (peso máximo absoluto en últimos 7 días + sparkline + 1RM Epley; tap → detalle). Modales nuevos: Resume con stats reales, Confirm Finish con stats + warn pendientes, Edit Muscles. **(e) Mental** preserva su arquitectura de 3 niveles (regla #3 revocada por usuario solo para refresh visual; timers/metronomes/diagramas intactos); micro-edición a `mentalShowHome` y `mentalShowSection` para `<h1.g-screen-title>` + subtitle. **(f) `app.js` simplificado**: removido bottom-bar/pinned-modules legacy, removido `renderModuleContent` y los renderers tipo `json`. **(g) `index.html`** removido Google Fonts CDN (regla #5), agregados scripts `icons.js` y `home.js`. **Validación:** `node -e new Function(...)` pasó para los 6 archivos JS. Schema DB sin cambios. **Skill global nueva:** `~/.claude/skills/claude-design-zip-review/SKILL.md` — flujo genérico de 3 fases (auditoría / iteración con Claude Design / implementación end-to-end) reusable para cualquier proyecto futuro. `sw.js → habitos-20260428-2` con `renderers/icons.js` y `renderers/home.js` agregados a ASSETS. |
| 2026-04-29 | `(pending)` | **Fix decimales en peso del add-set row:** `pesoInput` pasa de `type='number'` a `type='text' inputmode='decimal' pattern='[0-9]*[.,]?[0-9]*'` para que iOS Safari muestre confiablemente el separador decimal (con `type='number'` ciertos layouts del numpad lo ocultaban). Parser ahora normaliza coma → punto antes de `parseFloat`. **Migración una-vez de músculos duplicados:** nueva `gymRunMuscleDedup()` que se dispara al entrar al módulo Gym y, si la flag `preferencias.gym_muscle_dedup_v1` no existe, agrupa nombres de músculo por "stem" (sin acentos, sin sufijo plural -es/-s) y, cuando un grupo tiene varias variantes (ej. `Aductor` vs `Aductores`), conserva la que tenga MÁS ejercicios atribuidos y reescribe `ejercicios.musculo_primario` en el resto. Nueva helper `gymMuscleStem()`. Idempotente (flag), no toca `db.js` ni schema; solo escribe en `ejercicios` y `preferencias`. `sw.js → habitos-20260429-1`. |
| 2026-04-28 | `e25ab3b` | **Tab Entrenar — gestión de sesiones finalizadas:** las cards de "Últimas sesiones" ahora son tappables y abren una vista detalle (header con rutina/nombre/fecha + stats duración/ejercicios/sets/volumen + lista por ejercicio con sets, peso lbs × reps, status). Nuevo link "Ver todas →" abre lista completa scrolleable de sesiones finalizadas. La vista detalle expone dos botones: **✏️ Editar duración** abre modal con input `datetime-local` de hora fin (hora inicio mostrada read-only); guarda `duracion_ms = fin − inicio` con validación (fin > inicio). **🗑️ Eliminar sesión** abre confirm modal con resumen completo (rutina, fecha, ejercicios, sets) + warn irreversible; cascade delete de sets + sesión (no toca ejercicios del directorio ni flow_*/preferencias). Caso de uso: limpiar rutinas de prueba creadas durante features y corregir duraciones cuando se olvida finalizar la sesión y queda con horas erróneas. Nuevas funciones: `gymRenderSessionDetail`, `gymRenderAllSessions`, `gymOpenEditDurationModal`, `gymOpenDeleteSessionModal`, `gymTsToDatetimeLocal`. Sin tocar `db.js`, `styles.css`, ni `mental.js`. `sw.js → habitos-20260428-3`. |
